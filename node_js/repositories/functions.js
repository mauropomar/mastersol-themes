...
const executeFunctionsButtons = async (req, objects) => {
    ...
    if(idbutton) {
        ...
        if (resultButton && resultButton.rows && resultButton.rows[0].fn_get_register) {
                        ...
                        if(!extra_params || extra_params == ''){
                            ...
                            if (resultParamsGraphic && resultParamsGraphic.rows[0].fn_get_register != null
                                ...
                                for(let i=0;i<resultParamsGraphic.rows[0].fn_get_register.length;i++){
                                    ...
                                    const resultDatatype = await pool.executeQuery('SELECT cfgapl.fn_get_register($1,$2)', param_datatype)
                                    //Buscar operadores para el tipo de dato
                                    const paramsOpers = ['cfgapl.datatypes_comp_operators',null,"WHERE id_datatypes = '"+resultParamsGraphic.rows[0].fn_get_register[i].id_datatype+"' "]
                                    const resultOpers = await pool.executeQuery('SELECT cfgapl.fn_get_register($1,$2,$3)', paramsOpers)
                                    if(resultOpers && resultOpers.rows){
                                        resultParamsGraphic.rows[0].fn_get_register[i]['operadores'] = resultOpers.rows[0].fn_get_register
                                    }
                                    //Buscar el nombre del tipo de dato
                                    if(resultDatatype && resultDatatype.rows)
